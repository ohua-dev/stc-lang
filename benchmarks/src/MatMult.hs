...
{-Code taken from: https://github.com/simonmar/monad-par/blob/master/examples/src/matmult/matmult.hs -}
...
module MatMult(matmultBench) where
...
multMatricesTrOhua :: Matrix -> Matrix -> IO Matrix
multMatricesTrOhua m1 m2 = do
  (result,_) <- runOhuaM (smap (\row -> pure [prodEscalar2 row col | col <- m2]) m1) []
  return result
...
matmultBench =
  bgroup
    "matmult-bench"
    [ bench "sequential" (nf (mult opt (mA size) (mB size)) chunk)
    , bench "ohua" (nfIO (multMatricesTrOhua (mA size) $ transpose $ mB size))
