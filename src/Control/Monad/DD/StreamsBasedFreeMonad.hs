...
module Control.Monad.DD.StreamsBasedFreeMonad
      -- ** Data types
  ( Unique
  , Var
  , SfMonad(..)
...
newtype FunctorTaggedBinding f =
  FunctorTaggedBinding QualifiedBinding
...
-- 'SfMonad'
liftSf ::
     (ReturnType f ~ SfMonad state ret, Typeable ret, ApplyVars f) => f -> Sf f
  ...
  map
    ...
    , ( DFRefs.smapGFun
      , StreamProcessor $ pure $ do
          ...
          let gWithFlush :: Generator IO (Vector Dynamic)
              gWithFlush =
                fmap
                  (V.fromList . (: [toDyn True]))
                  (gen :: Generator IO Dynamic) `mappend`
                [ V.fromList
    ...
    , ( DFRefs.collectG
      , StreamProcessor $ pure $ do
          ...
          sendUntyped $ injectFunctor x (fromList l :: Generator IO Dynamic))
    , ( DFRefs.repeat
      , StreamProcessor $ do
          stateStore <- liftIO $ newMVar Nothing
          let recieveVals = recieveWhereUntyped (/= 0)
          pure $ withIsAllowed $ do
            b <- recieve 0
            if b
              then do
                vals <- liftIO (takeMVar stateStore) >>= maybe recieveVals pure
                liftIO $ putMVar stateStore $ Just vals
                send vals
              else liftIO $ modifyMVar_ stateStore (const $ pure Nothing))
    , ( DFRefs.isJust
      , StreamProcessor $ pure $ withIsAllowed $ send =<<
        (isJust . extractFunctor <$> recieveUntyped 0))
        -- , ( DFRefs.ndMerge
        --   , StreamProcessor $
        --     pure $
        --     withIsAllowed $ do
          ...
          pure $ do
            currGeneratorChan <-
              liftIO (takeMVar chanStore) >>= \case
                Nothing -> do
                  c <- liftIO newChan
                  send (chanToGenerator c :: Generator IO Dynamic)
                  pure $ liftIO . writeChan c
                Just f -> pure f
            v <- extractFunctor <$> recieveUntyped 0
            currGeneratorChan v
            liftIO $ putMVar chanStore $
              if isNothing v
                then Nothing
...
evaluateAST ::
     MonadStream m => ASTM s (Var a) -> (FunctionDict s m, L.Expression)
evaluateAST (ASTM m) = (dict, build e)
  where
    ...
    go ::
         MonadStream m
      => ASTAction s (EvalASTM s m (Var a))
      -> EvalASTM s m (Var a)
    ...
    go (If v then_ else_ cont) = do
      ...
      tellMut $
        L.Let
          (Direct rb)
          (L.Var (L.Sf ARefs.ifThenElse Nothing) `L.Apply` L.Var (L.Local vbnd) `L.Apply`
    ...
    go (Generate inner cont) = do
      ...
      tellMut $
        L.Let
          (Direct rb)
          (L.Var (L.Sf ARefs.generate Nothing) `L.Apply` L.Lambda "_" innerE)
      cont rv
      ...
      case source of
        G.LocalSource t@G.Target {G.index = i}
          ...
          | otherwise -> do
            opid <- generateId
            pure
              ( pure G.Operator {G.operatorId = opid, G.operatorType = dN i}
              , [ a
                    { G.source =
                        G.LocalSource G.Target {G.operator = opid, G.index = -1}
                    }
                , G.Arc
                    { G.target = G.Target {G.operator = opid, G.index = 0}
...
recieveSource ::
     MonadStream m => StreamM m a -> Reciever m (Packet a) -> StreamM m a
...
recieveWhereUntyped ::
     MonadStream m => (Int -> Bool) -> StreamM m (Vector Dynamic)
...
runFunc statRef funcName (SfRef (Sf f _) accessor) st = do
  ...
  pure $ (Just $ \s -> readIORef stateRef <&> \a -> s & accessor .~ a, ) $ pure $ do
...
mountStreamProcessor process ctxInput inputs outputs = do
  runReaderT (runExceptT $ runStreamM safeProc) (ctxInput, inputs, outputs) >>= \case
    ...
    Left (Just i) ->
      error $
      printf "There were packets left over in port %d when a processor exited" i
...
prettyExecutionException (ExecutionException errors) =
  unlines
    [ Pl.intercalate
      "\n  "
      ((e ++ " in ") : map showOp (take maxNumExceptionSources opList) ++
  ...
  where
    ...
    errMap = Map.fromListWith (++) $ map (\(op, e) -> (show e, [op])) errors
...
toOutputManager v
  ...
  where
    (argMap, unifiedArcsMap) =
      IMap.partitionWithKey (\k _ -> k > 0) $ IMap.fromListWith (++) $
      fmap (second pure) v
             ...
             operatorCode
               | nthNS == qbNamespace operatorType =
                 StreamProcessor $ pure $ do
                   input <- recieve 0
                   sendUntyped $ input V.!
                     read (toS $ unwrap $ qbName operatorType)
               | otherwise =
...
sf2 =
  call
    (liftSf (\T -> sfm $ liftIO (putStrLn "Executing sf2") >> return [0 .. 20]))
    stag

sf3 :: Var T -> Var Int -> ASTM s (Var Int)
sf3 =
  call
    (liftSf (\T i -> sfm $ liftIO (putStrLn "Executing sf3") >> return (succ i)))
    stag
