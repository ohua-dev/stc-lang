...
someFunc xs = runState (smap (bar .@. foo) xs)

-- someFunc :: [Int] -> S Int -> IO ([Int], S Int)
-- someFunc xs = runStateT (smap (bar .@. foo) xs)


-- someFunc :: [Int] -> IO ()
-- someFunc xs = do
--   print $ runState (smap (bar .@. foo) xs) $ S 0 0 0
  -- print $ runState (smap (bar . foo) xs) $ S 0 0 0
  -- print $ runState (smap (\x -> bar $ foo x) xs) $ S 0 0 0
