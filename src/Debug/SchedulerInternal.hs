...
sched _doSync queue t = loop t
 where
  loop t = case t of
    New a f -> do
      printf "cpu %d received NEW\n" $ no queue
      r <- newIORef a
      loop (f (IVar r))
    Get (IVar v) c -> do
      printf "cpu %d received GET\n" $ no queue
      e <- readIORef v
      case e of
         Full a -> loop (c a)
         _other -> do
    ...
    Put (IVar v) a t  -> do
      ...
      printf "cpu %d running continuation\n" $ no queue
      loop t
    Fork child parent -> do
         printf "cpu %d received FORK\n" $ no queue
