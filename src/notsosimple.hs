{-# LANGUAGE ExplicitForAll #-}
{-# LANGUAGE ScopedTypeVariables #-}
module Simple where

import Control.Monad.State hiding (state)
import Control.Monad.Trans.State (state)

data S s = S { s1 :: s, s2 :: s, s3 :: s} deriving (Show)

-- type SF s a = a -> StateT (S s) IO a
type SF s a = a -> State (S s) a

foo :: Int -> State Int Int
foo x = do
  s <- get
  put $ s+2
  return $ x+2

bar :: Int -> State Int Int
bar x = do
  s <- get
  put $ s+3
  return $ x*3

sfc1 :: forall a s.(a -> State s a) -> a -> State (S s) a
sfc1 f x = q where
  f' = f x -- create the state monad
  -- create a function that requires the global state and uses the state monad f'
  -- from the lexical scope to execute f' over only part of the state and then
  -- rebuilds the global state
  q' :: (S s) -> (a, S s)
  q' gs0 = (y, gs1) where
      S s1 s2 s3 = gs0
      (y, small) = runState f' s1
      gs1 = S small s2 s3
  -- finally we must return a new state monad that encapsulates the above
  -- computation.
  q = state q'

sfc2 :: forall a s.(a -> State s a) -> a -> State (S s) a
sfc2 f x = q where
  f' = f x
  q' :: (S s) -> (a, S s)
  q' gs0 = (y, gs1) where
      S s1 s2 s3 = gs0
      (y, small) = runState f' s2
      gs1 = S s1 small s3
  q = state q'

infixr 9 .@.
(.@.) :: SF s a -> SF s a -> SF s a
sf .@. sg = \x ->
              (sg x) >>= sf

someFunc :: Int -> S Int -> (Int, S Int)
someFunc x = runState (((sfc2 bar) .@. (sfc1 foo)) x)

main :: IO ()
main = do
  print $ runState (((sfc2 bar) .@. (sfc1 foo)) 10) $ S 0 0 0
